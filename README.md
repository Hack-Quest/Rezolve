...
Facts in the Decision Brief that **never appeared in the raw alert** — synthesized by the agents cross-referencing internal databases.
...
| **Investigator** | gemma3:1b (local) | Diagnose the attack + cite the matching CVE | `cve_database.json` (60 entries) |
| **Impact** | llama3.2:3b (local) or llama-3.3-70b (Groq) | Map the event to a corporate asset + assess business risk | `asset_inventory.json` (60 assets) |
| **Commander** | llama3.2:3b (local) or llama-3.3-70b (Groq) | Generate executive headline + 3 grounded action items | `playbooks.json` (50 playbooks) |
...
This hits every endpoint, validates the full 10-event SSE sequence (CVE cited, real asset matched, three concrete actions) actually fired — not just that the pipeline didn't crash. Run this before every demo.
...
├── shared/
│   └── data/
│       ├── cve_database.json           # 60 mock CVE entries
│       ├── asset_inventory.json        # 60 mock corporate assets
│       ├── playbooks.json              # 50 mock incident response playbooks
...
│       │   ├── impact.py               # Agent 3: asset + business impact 
...
| **CVE citation enforcement** | If the model's diagnosis omits the CVE ID despite the prompt rule, the matched CVE record is appended automatically |
